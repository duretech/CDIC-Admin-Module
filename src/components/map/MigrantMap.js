...
//import * from "leaflet-dvf/dist/leaflet-dvf.min.js";
...
//const geoJsonData = require("../../assets/shapeFile/AFG_IRN_PAK.json");
...
const MigrantMap = ({ mapKey, mapHeight }) => {
  //console.log(mapKey);
  ...
  const resetMap = () => {
    ...
    //currentMapReference.setZoom(mapDefaultZoom);
    //currentMapReference.setView(mapCenter);
    //console.log(migrantLayer);
    if (migrantLayer !== null) {
      migrantLayer.destroy();
    }

    setMigrantLayer(null);
    AnimatedMigrantLayer(mapRefObj);
  ...
  const exportMap = (e) => {
    ...
    html2canvas(document.getElementById("migrantMap1"), {
      allowTaint: true,
      useCORS: true,
  ...
  let mapGeoJsonStyle = (feature) => {
    ...
    let featureValue = mapIndicatorData[feature.properties.UID];
  ...
  const onEachFeature = (feature, layer) => {
    ...
    if (feature.properties && feature.properties.NAME_0) {
      ...
      let toolTipContent = `<b>Total Migrants in ${
        feature.properties.NAME_0
      }: ${
        mapIndicatorData[feature.properties.UID] !== undefined
          ? Number(mapIndicatorData[feature.properties.UID])
          : "N/A"
      }</b>`;
  ...
  const AnimatedMigrantLayer = (mapRef) => {
    ...
    if (migrantLayer === null) {
      ...
      migrationLayer.setData(migrantData);
    }
  };
    ...
    <>
      ...
      {showMap === true ? (
        ...
        >
          ...
          ></GeoJSON>
          {/* <AnimatedMigrantLayer key={currentMapKey + 1} /> */}
          <MapReferenceMarker />
